...
Shopify app that captures newsletter form submissions into Supabase (Postgres +
Prisma) using a schema-flexible Json column so new Shopify form fields never
require a migration.

## Start here

- **Architecture & design notes** — see [`PROJECT_MEMORY.md`](./PROJECT_MEMORY.md)
- **Theme install snippet** — [`theme/snippets/dreamlit-newsletter.liquid`](./theme/snippets/dreamlit-newsletter.liquid)
- **Env vars** — [`.env.example`](./.env.example)

## Setup status

Tracked against this repo's current state (updated 2026-04-21):

- [x] Scaffolded with `shopify app init` (Remix template)
- [x] Prisma schema switched to PostgreSQL + `NewsletterSubmission` / `RawPayloadLog` / `Session`
- [x] Supabase env in `.env` (DATABASE_URL pooled `:6543`, DIRECT_URL direct `:5432`)
- [x] `npx prisma migrate dev --name init` applied (migration `20260421085954_init`)
- [x] Prisma client generated (v6.19.3)
- [x] Supabase agent skills installed (`.claude/skills/supabase`, `.claude/skills/supabase-postgres-best-practices`) — available after Claude Code reload
- [ ] `INGEST_API_TOKEN` set in `.env` (needed only for the direct-API route)
- [ ] `shopify app dev` run (links Partner app, fills `SHOPIFY_API_KEY`/`SHOPIFY_API_SECRET`/`SHOPIFY_APP_URL`)
- [ ] `shopify app deploy` run (pushes `[app_proxy]` + webhook subs + scopes to Partner dashboard)
- [ ] Theme snippet installed on a test store and rendered
- [ ] End-to-end smoke test (form submit → row visible in Supabase `NewsletterSubmission`)
- [ ] Vercel deploy with env vars

## TL;DR deploy (full sequence)

```bash
# 1. Environment (DONE above — skip if .env is already populated)
cp .env.example .env
# Fill DATABASE_URL (Supabase pooled, :6543), DIRECT_URL (Supabase direct, :5432),
# INGEST_API_TOKEN (openssl rand -hex 32).

# 2. Database (DONE — Session/NewsletterSubmission/RawPayloadLog live in Supabase)
npx prisma migrate dev --name init

# 3. Shopify dev (interactive, links to your Partner app). Must run in your own
#    terminal — the Partner login flow opens accounts.shopify.com in a browser.
shopify app dev

# 4. Deploy config (App Proxy + webhooks + scopes) to Partner Dashboard
shopify app deploy

# 5. Install on a test store, then paste into the theme:
...
| Path                              | Purpose                                  | Auth                        |
| --------------------------------- | ---------------------------------------- | --------------------------- |
| `POST /apps/newsletter`           | App Proxy target (storefront forms)      | Shopify-signed (HMAC-SHA256, hex) |
| `POST /api/newsletter`            | Direct API (non-storefront callers)      | `Authorization: Bearer ...` |
| `POST /webhooks/customers/create` | Fallback capture when customer is made   | Shopify webhook HMAC (base64) |

## Data model

See [`prisma/schema.prisma`](./prisma/schema.prisma). Base columns
(`id`, `shop`, `email`, `createdAt`) are real columns; everything else lands
in `meta` (normalized Json) and `rawPayload` (verbatim Json). Unique on
`(shop, email)` when email is present.

---

Original Shopify Remix template docs follow below. They're kept for reference
but the DreamLitxSimplSo-specific docs above take precedence.
